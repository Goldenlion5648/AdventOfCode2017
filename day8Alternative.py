...
for i, com in enumerate(commands):
    index = com.index(" ")
    ifIndex = commands[i].index("if") + 3
    after =com.index(" ", ifIndex)
    firstVar = com[0:index]
    # print(i)
    commands[i] = "registers['" + com[:index] + "']" + com[index:ifIndex] + "registers['" + com[ifIndex:after] + "']" + com[after:]
    commands[i]= commands[i].replace("dec", "-=")
    commands[i]= commands[i].replace("inc", "+=")
    print(commands[i])
    ifIndex2 = commands[i].index("if")
    commands[i] = commands[i][ifIndex2:] + ": " + commands[i][0:ifIndex2]
    print(commands[i])
    exec(commands[i])
